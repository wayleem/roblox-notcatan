...
import { create, del, flush, update } from "shared/actions";
...
export function on_player_join(player: Player) {
  ...
  store.dispatch(update<PlayerState>(playerId, "resources", { wheat: }))
}
...
function update(key: keyof Resource, count: number): Resource {
  old_resource[key] += count
  return old_resource
}
