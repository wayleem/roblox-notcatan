...
for (let i = 0; i < 4; i++) {
  ...
  if (i < 1) {
    resources.push(
      { wheat: 0, sheep: 0, ore: 0, wood: 0, brick: 0 }
    )
  }
}
...
function create_hexagon(q: number, r: number, tileSize: number, resource: Resource, token: number): void {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const vertexVector = new Vector3(center.X + tileSize * math.cos(angle), center.Y, center.Z + tileSize * math.sin(angle));
    const nextAngle = (2 * math.pi * (i + 1)) / 6;
    const nextVertexVector = new Vector3(center.X + tileSize * math.cos(nextAngle), center.Y, center.Z + tileSize * math.sin(nextAngle));
