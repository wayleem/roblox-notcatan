...
Players.LocalPlayer.CharacterAdded.Connect((c) => {
	print("client vertices: ", Object.values(localStore.getState().board.vertices));

	print("client's resources: ", localStore.getState().player.resources);
});
...
button.MouseClick.Connect(() => {
	wait(1);
	print("updated client's resources: ", localStore.getState().player.resources);
});

clientButton.MouseClick.Connect(() => {
	/*
  const vertices = localStore.getState().board.vertices;
